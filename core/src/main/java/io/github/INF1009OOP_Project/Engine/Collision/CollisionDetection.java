...
public class CollisionDetection {
	public ArrayList<Collision> detect(ArrayList<Collidable> all) {// takes in collidable array from collisionManager
		...
		for (int i = 0; i < all.size(); i++) {// loops through collidable array
			...
			for (int j = i + 1; j < all.size(); j++) {// loop to check if first object collided with any of the
														// remaining object
				Collidable b = all.get(j);
				if (a.getBounds().intersects(b.getBounds())) {// if bounds intersect add both objects as a collision
																// object into collisions
